<x-app-layout>
        ...
        <form action="/posts" method="POST" enctype="multipart/form-data">
            ...
            <div class="depth">
                <h2>最大水深</h2>
                    <input type="text" name="post[depth]" placeholder="整数" value="{{ old('post.depth') }}"/>
                ...
                <h2>評価</h2>
                 <select name="post[evaluation_id]">
                    @foreach($evaluations as $evaluation)
                        <option value="{{ $evaluation->id }}"{{old('post.evaluation_id')== $evaluation->id  ? 'selected' : ''}}>{{ $evaluation->evaluation_name }}</option>
                    @endforeach
                </select>
            </div>
